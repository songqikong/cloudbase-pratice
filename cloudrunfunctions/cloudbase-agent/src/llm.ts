...
export class LLMCommunicator {
  ...
  constructor(botContext: BotContext<any>, options: LLMCommunicatorOptions) {
  ...
  private initModel() {
    let openaiFunc: any = createOpenAI;
   ...
   * 处理streamText输出
   */
  private dealStreamText(streamPart: ai.TextStreamPart<ai.ToolSet> | ToolResult): {
    type: string;
    content: string;
  } {
  ...
  async stream({
    ...
    try {
      await this.streamText(messages, (streamPart: ai.TextStreamPart<ai.ToolSet>) => {
        ...
        if (tc.type === 'text' && tc.content) {
          ...
          callMsg.push(result);
          this.botContext.bot.sseSender.send(`data: ${JSON.stringify(result)}\n\n`);
        } else if (streamPart.type === 'step-finish' && streamPart.finishReason === 'tool-calls') {
          // 工具调用结束
          result = {
            ...result,
            content: '\n',
          ...
          result = {
            ...
            error: {
              name: 'LLMError',
              message: streamPart.error as any,
            },
          };
